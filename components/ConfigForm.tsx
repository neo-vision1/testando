...
const ConfigForm: React.FC<ConfigFormProps> = ({ drone, config, isExpanded, onToggle, onSave }) => {
  ...
  return (
    <div className="border border-slate-800 rounded-lg overflow-hidden bg-slate-900/50 mb-3 transition-all duration-300">
      <button 
      ...
      >
        <div className="flex items-center gap-3">
          <span className={`text-sm ${isExpanded ? 'text-blue-400' : 'text-slate-400'}`}>
            {isExpanded ? '▼' : '▶'}
          </span>
          <span className="font-semibold text-slate-200">{drone.name}</span>
        </div>
        <span className="text-xs text-slate-500 font-mono hidden sm:inline-block">
          {currentConfig.playbackId.substring(0, 12)}...
        </span>
      </button>

      {isExpanded && (
        <form onSubmit={handleSubmit} className="p-4 border-t border-slate-800 bg-slate-950/80 animate-in fade-in slide-in-from-top-2">
          
          <div className="mb-4 bg-blue-950/20 border border-blue-900/50 rounded p-3 text-sm text-blue-200">
            <h5 className="font-bold mb-1 flex items-center gap-2">
              📡 Instruções de Streaming
            </h5>
            <p className="text-blue-300/80 text-xs">Configure seu software de transmissão (OBS, vMix) com os dados abaixo.</p>
          </div>

          <div className="space-y-4">
            {/* Stream Key Input */}
            <div>
              <label className="block text-xs uppercase text-slate-400 font-bold mb-1 ml-1">Chave de Stream (RTMP Key)</label>
              <div className="flex rounded-md shadow-sm overflow-hidden border border-slate-700 focus-within:border-blue-500 focus-within:ring-1 focus-within:ring-blue-500">
                <input
                  ...
                  className="flex-1 block w-full bg-slate-950 px-3 py-2 text-sm text-white placeholder-slate-600 focus:outline-none"
          ...
          <div className="mt-6 flex justify-end">
            <button 
              type="submit" 
              className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded shadow-lg shadow-blue-900/20 transition-all hover:scale-105 active:scale-95"
            >
